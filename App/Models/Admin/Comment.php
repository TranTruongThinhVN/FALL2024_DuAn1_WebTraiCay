...
class Comment extends BaseModel
{
    ...
    {
        $result = 0; // Khởi tạo kết quả mặc định
        try {
            $sql = "SELECT COUNT(id) as total FROM comments";
            $conn = $this->_conn->MySQLi();
            $stmt = $conn->prepare($sql);
            $stmt->execute();
            $row = $stmt->get_result()->fetch_assoc(); // Lưu kết quả vào $row
            $result = $row['total']; // Gán giá trị từ $row['total'] cho $result
        } catch (\Throwable $th) {
            error_log('Lỗi khi lấy tổng số bình luận: ' . $th->getMessage());
        }
    ...
    public function searchComments($keyword)
    {
        ...
        try {
            ...
            $searchTerm = '%' . $keyword . '%';
            $stmt->bind_param('ssss', $searchTerm, $searchTerm, $searchTerm, $searchTerm); // 'ssss' cho 4 tham số kiểu string

            $stmt->execute();
            $result = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
        } catch (\Throwable $th) {
            error_log('Lỗi khi tìm kiếm bình luận: ' . $th->getMessage());
        }
    ...
    public function getTotalSearchComments($keyword)
    {
        $result = 0;
        try {
            $sql = "SELECT COUNT(*) as total FROM " . $this->table . " WHERE content LIKE ?";
            $conn = $this->_conn->MySQLi();
            $stmt = $conn->prepare($sql);

            // Thêm ký tự '%' vào trước và sau từ khóa để tìm kiếm
            $searchTerm = '%' . $keyword . '%';
            $stmt->bind_param('s', $searchTerm); // 's' cho tham số kiểu string

            $stmt->execute();
            $result = $stmt->get_result()->fetch_assoc()['total'];
        } catch (\Throwable $th) {
            error_log('Lỗi khi tìm kiếm tổng bình luận: ' . $th->getMessage());
        }
    ...
    public function getCommentByProductId($id, $offset, $commentsPerPage)
    {
        $result = [];
        try {
            // Câu lệnh SQL để lấy bình luận của sản phẩm, với LIMIT và OFFSET
            $sql = "SELECT comments.*, 
                ...
                LIMIT ?, ?;"; // Thêm LIMIT và OFFSET

            $conn = $this->_conn->MySQLi();
            $stmt = $conn->prepare($sql);
            $stmt->bind_param('iii', $id, $offset, $commentsPerPage); // Bind các tham số (id, offset, commentsPerPage)
            $stmt->execute();
    ...
    public function getCountCommentByProductID($id)
    {
        $result = [];
        try {
            $sql = "SELECT COUNT(*) AS total_comments FROM comments WHERE product_id = $id";
            ...
            if ($query) {
                ...
                $countImages = $this->getCountImagesByProductId($id);
            } else {
